{
  ...
  inputs = {
    ...
    packages = {
      url = "github:almostnobody/tobii_eye_tracker_linux_installer?ref=packages";
      flake = false;
    };
  };

  outputs = { self, nixpkgs, packages }: {
    packages.x86_64-linux = with import nixpkgs { system = "x86_64-linux"; };
      rec {
        tobii-usbservice = stdenv.mkDerivation {
          ...
          buildPhase = ''
            ${pkgs.dpkg}/bin/dpkg-deb -x \
              ${packages}/tobiiusbservice_l64U14_2.1.5-28fd4a.deb tobiiusbservice
          '';
          installPhase = ''
            mkdir -p $out/{bin,lib}
        ...
        tobii-engine = stdenv.mkDerivation {
          ...
          buildPhase = ''
            ${pkgs.dpkg}/bin/dpkg-deb -x \
              ${packages}/tobii_engine_linux-0.1.6.193_rc-Linux.deb tobii-engine
          '';
          installPhase = ''
            mkdir -p $out/{bin,lib}
        ...
        tobii-config = stdenv.mkDerivation {
          ...
          buildPhase = ''
            ${pkgs.dpkg}/bin/dpkg-deb -x \
              ${packages}/tobii_config_0.1.6.111_amd64.deb tobii-config
          '';
          installPhase = ''
            mkdir -p $out
        ...
        tobii-manager = stdenv.mkDerivation {
          ...
          buildPhase = ''
            ${pkgs.dpkg}/bin/dpkg-deb -x \
              ${packages}/TobiiProEyeTrackerManager-2.1.0.deb tobii-manager
          '';
          installPhase = ''
            mkdir -p $out
